...
begin
  ...
  if (FBuffer.Width = AWidth) and (FBuffer.Height = AHeight) and (dirtyRectsCount > 0)  then
  begin
    FBuffer.Map(TMapAccess.maReadWrite, wBitmapData);    // Getting the bitmap data pointer
    try
      ...
      begin
        begin
          ...
          begin
            ...
            begin
              wOffset := (((dirtyRects[c].y + i) * Awidth) + dirtyRects[c].x + j) * _BytesPercolor; // calculate offset
        ...
        InvalidateRect(RectF(0,0,Awidth,Aheight));      // update
      end;
    finally
...
begin
  if Browser <> nil then
  with AbsoluteToLocal(Screen.MousePos).Round do
  begin
    event.x := X;
    event.y := Y;
    event.modifiers := getModifiers(Shift);
  ...
  begin
    ...
    begin
      ...
      if FBuffer = nil then
        ...
        if Trunc(Width) <> FBuffer.Width then
        begin
